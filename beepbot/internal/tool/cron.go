...
func (t *CronTool) Description() string {
	return `定时任务工具 - 用于创建在未来特定时间触发的任务。

【重要】message 参数说明：
- message 是定时触发时发送给你（智能体）的指令
- 触发时，你会收到这条 message 作为用户消息
- 你需要根据 message 内容执行相应操作

【正确示例】
用户请求："每天早上9点提醒我开会"
→ message: "现在是早上9点，请提醒用户开会"
→ 触发后你会执行：发送提醒消息给用户

用户请求："每小时检查一次服务器状态"
→ message: "请检查服务器状态并报告结果"
→ 触发后你会执行：检查服务器状态并报告

用户请求："每分钟给我发送一句话"
→ message: "请给用户发送一句问候或有趣的话"
→ 触发后你会执行：发送一句话给用户

【错误示例】❌
用户请求："每分钟给我发送一句话"
→ message: "每分钟给用户发送一句话" ← 错误！这会导致你重复创建定时任务

【禁止事项】
- message 中不要包含"创建定时任务"的指令
- message 中不要包含 cron 表达式
- message 应该是你触发后要执行的具体动作，而不是任务描述

Cron表达式格式（6字段）: 秒 分 时 日 月 周
示例:
- "0 30 * * * *" - 每小时30分执行
	...
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"message": map[string]any{
				"type":        "string",
				"description": "触发时发送给你的指令。这是你给未来自己的指令，触发时你会收到这条消息作为用户消息。例如：'请提醒用户开会'、'请检查服务器状态'。不要在message中包含创建定时任务的指令！",
			},
